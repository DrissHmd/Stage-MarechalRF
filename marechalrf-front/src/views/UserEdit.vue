  ...
  <div class="user-edit-page">
    <div class="user-edit-container">
      ...
      <form @submit.prevent="updateUser" class="user-form">
        ...
        <!-- Champ 'Rôle' visible uniquement si l'utilisateur est admin -->
        <div v-if="isAdmin">
          <label for="role">Rôle:</label>
          <select v-model="userForm.roleId" id="role">
        ...
        <div class="buttons">
          ...
          <!-- Bouton 'Supprimer' visible uniquement si l'utilisateur est admin -->
          <button v-if="isAdmin" type="button" @click="deleteUser" class="delete-button">Supprimer l'utilisateur</button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  data() {
    return {
      ...
      roles: [],
      isAdmin: false, // Variable pour gérer l'état admin
    };
  },
  setup() {
  ...
  created() {
    this.checkIfAdmin(); // Vérification du rôle admin
    this.fetchRoles();
    this.fetchUser();
  },
  methods: {
    checkIfAdmin() {
      // Vérifie si l'utilisateur est admin en se basant sur le localStorage
      this.isAdmin = localStorage.getItem('roleId') === "2";
    },
    async fetchRoles() {
      try {
    ...
    async fetchUser() {
      ...
      if (!userId) {
        // Si pas d'ID dans l'URL, on récupère l'ID depuis le localStorage (ou Vuex)
        userId = localStorage.getItem('userId'); // Assure-toi que cet ID est stocké lors de la connexion de l'utilisateur
      }
